...
class LFGenGenerator(AbstractGenerator):
    ...
    def generate_outputs(self, inputs):
        batch_size, path_len, _ = inputs.shape
        out_len = self.config['out_len']
        outputs = []
        rho = np.array([[self.rho(t,s) for s in range(path_len)] for t in range(out_len)])
        
        return np.einsum('bsd,ts->btd',inputs,rho)
...
class LinearRNNEncDecGroundTruth(LFGenGenerator):
    """
    The ground truth for a linear RNN. 
    """
    name = 'LinearRNNGroundTruth'
...
class TwoPart(LFGenGenerator):
    ...
    def generate_inputs(self, batch_size, path_len):
        ...
        for center, sigma in zip(self.config['centers'], self.config['sigmas']):
            interval_length = int(3/sqrt(sigma))
            input = 0.1 * np.random.normal(size=(batch_size,
                                                path_len,
                                                self.config['input_dim'],))
            if center - interval_length > 0:
                input[:, :center - interval_length,:] = 0
            input[:,center+interval_length:,:] = 0
            inputs.append(input)

        input1 = 0.1 * np.random.normal(size=(batch_size,
                                                path_len,
                                                self.config['input_dim'],))

        for center, sigma in zip(self.config['centers'], self.config['sigmas']):
            interval_length = int(3/sqrt(sigma))
            input1[:,center-interval_length:center+interval_length,:] = 0
